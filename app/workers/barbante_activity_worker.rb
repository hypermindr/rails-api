class BarbanteActivityWorker
  include Sidekiq::Worker
  sidekiq_options :queue => :barbante_activity, :retry => 1, :failures => :exhausted

  def perform(external_user_id, external_product_id, activity, created_at, anonymous)
    result = Modules::Barbante.process_activity(external_user_id, external_product_id, activity, created_at, anonymous)
